...
export const view_roster = async (req, res) => {
    const { roomNumber } = req.params;

    try {
        const roster = await rooms_model.view_roster(roomNumber);

        return res.status(200).json({ success: true, error: false, message: "Successfully fetched room roster", roster: roster })
    } catch (error) {
        return res.status(500).json({ success: false, error: true, message: "Failed to fetch room roster" })
    }
}